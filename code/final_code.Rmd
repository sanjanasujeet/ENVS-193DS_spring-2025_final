...
2024.
Daily sea surface temperature in Santa Barbara channel between 1982 and 2023 ver 3.
...
Another important thing to include would be summary statistics, like mean standard error and range, for each group, this would convey important information in the direction and magnitude of differences as well as how spread out the data is. \*\* Effect size \### c.
Suggestions for rewriting

Sites farther from the river headwaters were associated with higher total annual nitrogen loads.
...
Nitrogen load varied significantly across different source types; sources \_ contributed more nitrogen than \_ the others which suggests that \_ is a key driver of elevated nitrogen levels in the system.
...
A follow up post-hoc Tukey's HSD test determined that \_ source( mean difference = average kg/yr, p = p-value ) had significantly higher nitrogen loads than \_ source(mean difference = average kg/yr, p= p-value ); no significant differences were found between the other sources.
...
ggplot(sst_clean, aes(x=month, y= mean_monthly_sst, group = year, color = year))+
  ...
  labs(x = "Month", y= "Mean monthly sea surface temperature (°C)")
```
...
### d. Table of models
| Model number | edge dist | season |              |  
|:------------:|:---:|:---------------:|:-------:|-----------------------------|  | 0            |     |                 |         | no predictors (null model)  |
| 1            |  X  |       X         |    X    | edge distance | 
| 2            |  X  |       X         |         | season     | | 3            |  X  |                 |    X    | all predictors (full model)             |    
...
Model0<- lm(sp ~ 1, data = nest_boxes_clean, family = binomial()) #no predictors
Model1<- lm(sp ~ edge_distance, data = nest_boxes_clean) #edge dist
Model2<- lm(sp ~ season, data = nest_boxes_clean) # season
Model3<- lm(sp ~ edge_distance+season, data = nest_boxes_clean) #both predictors
...
In the affective visuals, my aim was still to communicate data but I was more focused towards a creative format.
